...
void test_sudoku_solve_simple(std::ostream &ost) {
  ...
  clear_grid(grid);
  ost << "*** GRID SHOULD START CLEAR:\n";
  print_sudoku_grid(ost, grid);
 
  // TEST ROW HANDLING
  skip=3;
  ost << "*** FIRST ROW FULL EXCEPT IN COLUMN NUMBER " << skip << ":\n";
  for(int i=0;i<9;++i) { 
  ...
  skip=4;
  ost << "*** FIRST COLUMN FULL EXCEPT IN ROW NUMBER " << skip << ":\n";
  ...
  ost << "*** TOP LEFT SUBGRID FULL EXCEPT FOR CELL IN ROW " << skiprow << " AND COL " << skipcol << ":\n";
